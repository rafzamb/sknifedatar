...
#'         modelo suministrado donde se almacenanan los modelos o workflows entrenados para cada series. Finalmente las columnas
#'         nested_model y calibration que guardan los n modelos entrenados para cada serie y las metricas de ajuste sobre la
#'         particion de test.
#'
#' @export
#'
#' @examples
multi_fit = function(serie, .prop, ...){
  ...
  table_time = time_data %>%
    ...
    dplyr::mutate(nested_model = purrr::pmap(eval(exp3), .f = function(...) {modeltime::modeltime_table(...)}),
           ...
           calibration = purrr::pmap(list(.data$nested_model, .data$nested_column), function(nested_model, nested_column) {

             nested_model %>%

               modeltime::modeltime_calibrate(new_data = rsample::testing(rsample::initial_time_split(.data$nested_column, prop = 0.9)))
  ...
  models_accuracy = mapply(function(calibracion, name_ts) {
    ...
    calibracion %>%
      ...
      dplyr::mutate(name_serie = .data$name_ts) %>%
